<div class="title-bar" data-responsive-toggle="example-animated-menu" data-hide-for="medium">
      <button class="menu-icon" type="button" data-toggle></button>
      <div class="title-bar-title">Učenje PHP-a</div>
    </div>

    <div class="top-bar" id="example-animated-menu" data-animate="hinge-in-from-top spin-out">
      <div class="top-bar-left">
        <ul class="dropdown menu" data-dropdown-menu>
          <li class="menu-text">Učenje PHP-a</li>
          <li><a href="index.php">Početna</a></li>
          <li>
            <a href="#">Osnove</a>
            <ul class="menu vertical">
              <li><a href="grid.php">Grid sustav</a></li>
              <li><a href="prijenosParametara.php?boja=green&font=Arial">Prijenos parametara</a></li>
             
            </ul>
          </li>
          <li>
            <a href="#">Osnove PHP-a</a>
            <ul class="menu vertical">
                <li>
                    <a href="#">Varijabla i tipovi podataka</a>
                    <ul class="menu vertical">
                        <li><a href="osnovephp/varijabla.php" target="_blank">Varijable</a></li>
                        <li><a href="osnovephp/operatori.php" target="_blank">Operatori</a></li>
                        <li><a href="osnovephp/prviProgram.php" target="_blank">prviProgram</a></li>
                        <li><a href="osnovephp/uvjetnoGrananjeIf.php" target="_blank">uvjetno grananje</a></li>
                        <li><a href="osnovephp/uvjetnoGrananjeSwitch.php" target="_blank">uvjetno grananje switch</a></li>
                        <li><a href="vizualniProgram.php" target="_blank">vizualni program</a></li>
                        <li><a href="pokusaj.php" target="_blank">pokusaj</a></li>


                    
                    </ul>
                </li>
                <li>
                    <a href="#">Petlje i nizovi</a>
                    <ul class="menu vertical">
                        <li><a href="petljeinizovi/nizovi.php" target="_blank">Nizovi</a></li>
                        <li><a href="petljeinizovi/ugradeniNizovi.php" target="_blank">Ugrađeni nizovi</a></li>
                        <li><a href="petljeinizovi/funkcijeINizovi.php" target="_blank">Funkcije i nizovi</a></li>
                        <li><a href="petljeinizovi/forpetlja.php" target="_blank">For petlja</a></li>
                        <li><a href="petljeinizovi/whilepetlja.php" target="_blank">while petlja</a></li>
                        <li><a href="petljeinizovi/doWhilepetlja.php" target="_blank">do while petlja</a></li>
                        <li><a href="petljeinizovi/forEach.php" target="_blank">for each petlja</a></li>

                        
                        


                    
                    </ul>
                </li>
                <li>
                    <a href="#">Forme</a>
                    <ul class="menu vertical">
                        <li><a href="getForma.php" >GET forma</a></li>
                        <li><a href="postForma.php" >POST forma</a></li>

                       

                        
                        


                    
                    </ul>
                </li>
            </ul>
          </li>
          <li>
            <a href="#">Zadaci</a>
            <ul class="menu vertical">
                <li>
                    <a href="#">Osnove</a>
                    <ul class="menu vertical">
                        <li><a target="_blank" href="zadaci/Z01.php?ime=Bosko&boja=green&font=Arial">Zadatak 1</a></li>
                        <li><a target="_blank" href="zadaci/Z02.php">Zadatak 2</a></li>
                        <li><a target="_blank" href="zadaci/Z03.php">Zadatak 3</a></li>
                        <li><a target="_blank" href="zadaci/Z04.php">Zadatak 4</a></li>
                        <li><a target="_blank" href="zadaci/Z05.php">Zadatak 5</a></li>
                        <li><a target="_blank" href="zadaci/Z07.php">Zadatak 6</a></li>
                        <li><a target="_blank" href="zadaci/Z08.php">Zadatak 8</a></li>
                        <li><a target="_blank" href="zadaci/Z08.php">Zadatak 9</a></li>
                        <li><a target="_blank" href="zadaci/Z10.php">Zadatak 10</a></li>
                        <li><a target="_blank" href="zadaci/Z11.php">Zadatak 11</a></li>
                        <li><a target="_blank" href="Z1.php">Zadatak 01</a></li>
                        <li><a target="_blank" href="Z2.php">Zadatak 02</a></li>
                        <li><a target="_blank" href="Z3.php">Zadatak 03</a></li>
                        <li><a target="_blank" href="Z5.php">Zadatak 05</a></li>
                        <li><a target="_blank" href="Z6.php">Zadatak 06</a></li>
                        <li><a target="_blank" href="Z7.php">Zadatak 07</a></li>
                        <li><a target="_blank" href="Z8.php">Zadatak 08</a></li>
                        <li><a target="_blank" href="Z9.php">Zadatak 09</a></li>
                        <li><a target="_blank" href="Z10.php">Zadatak 10</a></li>
                        <li><a target="_blank" href="Z11.php">Zadatak 11</a></li>
                        <li><a target="_blank" href="Z12.php">Zadatak 12</a></li>
                        <li><a target="_blank" href="Z13.php">Zadatak 13</a></li>
                       
                      
                    </ul>
                </li>
            </ul>
          </li>
        </ul>
      </div>
    </div>
    <!-- End izbornik -->